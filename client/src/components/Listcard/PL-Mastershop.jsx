...
const PLMastershop = () => {
  ...
  const displayedProducts = shuffledProducts.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
          ...
          {totalPages > 1 && (
            <div className="flex justify-center mt-4 space-x-2">
              {Array.from({ length: totalPages }, (_, index) => (
                <button
                  key={index + 1}
                  className={`px-4 py-2 rounded ${currentPage === index + 1 ? "bg-color-custom-1 text-white" : "bg-gray-200"}`}
