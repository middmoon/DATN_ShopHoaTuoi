...
// store: vnp_TxnRef, vnp_Amount, vnp_OrderInfo. vnp_OrderType, vnp_CreateDate, vnp_BankCode
...
//   console.log(`create_payment_url req.body =  ${JSON.stringify(req.body)}`);
...
//   console.log(`vnpUrl req.body = ${vnpUrl}`);
...
router.post(
  ...
  asyncHandler(async function (req, res, next) {
    ...
    let orderId = req.orderInfo;
    // let txnRef = req.txnRef;
    let txnRef = req.body.txnRef;
    let amount = req.body.amount;
    let bankCode = req.body.bankCode;
    ...
    await OrderService.updateOrderInfo(req.body.orderId, {
  ...
  asyncHandler(async function (req, res, next) {
    ...
    if (secureHash === signed) {
      ...
      if (vnp_Params.vnp_ResponseCode === "00") {
        ...
        res.redirect(`http://localhost:${process.env.CIENT_PORT}/payment-success`);
      } else {
        await OrderService.updateOrderInfo(orderId, {
          paymentStatus: "Thất bại",
          paymentInfo,
        });
        res.redirect(`http://localhost:${process.env.CIENT_PORT}/payment-failure`);
      }
    } else {
      await OrderService.updateOrderInfo(orderId, {
        paymentStatus: "Thất bại",
        paymentInfo: {
          vnp_ResponseCode: "checksum_error",
        },
      });
      res.redirect(`http://localhost:${process.env.CIENT_PORT}/payment-failure?error=checksum`);
    }
  })
);
...
router.get("/vnpay_ipn", function (req, res, next) {
  ...
  let hmac = crypto.createHmac("sha512", secretKey);
  //   let signed = hmac.update(new Buffer(signData, "utf-8")).digest("hex");
  let signed = hmac.update(new Buffer.frpm(signData, "utf-8")).digest("hex");
